defmodule MessagingService.Producer.MMSAdapterTwilio do
  require Logger

  def process(%{type: :mms} = message) do
    case send_mms(message) do
      {:ok, _response} ->
        :ok

      {:error, reason} ->
        Logger.error("Failed to send MMS via Twilio for #{message.id} #{inspect(reason)}")
        {:error, reason}
    end
  end

  defp send_mms(message) do
    url = twilio_url()
    auth = twilio_auth()

    # For MMS, we need to include media URLs
    media_urls = Enum.map(message.attachments, & &1)
